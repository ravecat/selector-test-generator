...
if [ ${COUNTBRANCHES} -ne "0" ]; then
  ...
  if [ -n ${MR_NUMBER_GITHUB} ]; then
    ...
    LIST_MR_GITHUB=`curl --silent "https://api.github.com/search/issues?q=head:${CI_COMMIT_REF_NAME}+repo:${GITHUB_OWNER}/${CI_PROJECT_NAME}" --header "Accept: application/vnd.github.v3+json"`;
